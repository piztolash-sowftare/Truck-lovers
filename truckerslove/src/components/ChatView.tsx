...
import { sendPrivateMessage, unmatchUser, blockUser } from '@/app/actions';
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] bg-[#020617] relative">
      ...
      <header className="p-5 border-b border-white/5 bg-slate-950/50 backdrop-blur-xl flex items-center justify-between z-20">
        <div className="flex items-center gap-3">
          ...
          <Link href={`/profile/${otherUser?.id}`} className="flex items-center gap-3 active:opacity-70 transition-opacity">
            ...
            <div>
              ...
              <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest flex items-center gap-1">
                <Truck size={8} className="text-red-500" /> {otherUser?.truckModel || 'Vezi Profil'}
              </p>
            </div>
          </Link>
        </div>
      ...
      <AnimatePresence>
        {showSettings && (
          ...
          >
            ...
            <button 
              onClick={async () => {
                if (confirm('Vrei să blochezi acest utilizator? Nu vei mai putea comunica.')) {
        ...
        {messages.map((msg: any) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-slide-up`}>
              <div className={`max-w-[85%] ${isMe ? 'items-end' : 'items-start'} flex gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                {!isMe && (
                  <div className="w-8 h-8 rounded-full bg-slate-800 border border-white/10 overflow-hidden flex-none shadow-xl relative">
                     {otherUser?.image ? <img src={otherUser.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-red-500 font-black text-[10px]">{otherUser?.name[0]}</div>}
                     <div className="online-dot !w-2 !h-2" />
                  </div>
                )}
                <div className="flex flex-col">
                  <div 
                    className={`p-4 rounded-[1.75rem] text-sm font-medium leading-relaxed shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)] ${
                      isMe 
                      ? 'bg-[#ff385c] text-white rounded-tr-none' 
                      : 'bg-slate-900 border border-white/5 text-slate-200 rounded-tl-none shadow-2xl'
                    }`}
                  >
                    {msg.content}
                  </div>
                  <span className={`text-[7px] font-black text-slate-700 uppercase tracking-widest mt-1.5 px-1 ${isMe ? 'text-right' : 'text-left'}`}>
