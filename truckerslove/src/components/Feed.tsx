...
export default function Feed({ initialPosts, currentUserId, isAdmin }: any) {
  ...
  const [isLiking, setIsLikeId] = useState<number | null>(null);
            ...
            <div className="p-7 space-y-6">
              <div className="flex items-center gap-8">
                <motion.button 
                    ...
                    const newPosts = posts.map((p: any) => {
                      if (p.id === post.id) {
                        return {
                          ...p,
                          likes: isLiked 
                            ? p.likes.filter((l: any) => l.userId !== currentUserId)
                  ...
                  className={`flex items-center gap-2.5 text-xs font-black transition-all ${post.likes.some((l: any) => l.userId === currentUserId) ? 'text-[#ff003c]' : 'text-slate-600'}`}
                >
