  ...
  if (!currentUser || currentIndex >= users.length) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center p-6">
        <Truck size={64} className="text-slate-800 mb-4" />
        <h2 className="text-xl font-bold text-white">Nu mai sunt tiriști în zonă!</h2>
        <p className="text-slate-500 uppercase text-[10px] font-black tracking-widest mt-2">Revino mai târziu sau schimbă radarul</p>
      </div>
    );
  }
      ...
      <AnimatePresence>
        ...
        >
          <div className="relative h-full w-full">
            {/* Action Badges */}
            <motion.div style={{ opacity: likeOpacity }} className="absolute top-10 left-10 z-30 border-4 border-emerald-500 px-6 py-2 rounded-2xl -rotate-12 bg-emerald-500/10 backdrop-blur-md">
              <span className="text-4xl font-black text-emerald-500 uppercase tracking-widest">MĂ BAG</span>
            </motion.div>
            <motion.div style={{ opacity: dislikeOpacity }} className="absolute top-10 right-10 z-30 border-4 border-red-500 px-6 py-2 rounded-2xl rotate-12 bg-red-500/10 backdrop-blur-md">
            ...
            <div className="w-full h-full bg-slate-800 flex items-center justify-center relative" onClick={nextPhoto}>
              {(() => {
                let images = [];
                try {
                  images = currentUser.images ? JSON.parse(currentUser.images) : [currentUser.image];
                } catch (e) {
