...
export default function SwipeCards({ initialUsers }: { initialUsers: User[] }) {
  ...
  const bgOpacity = useTransform(x, [-200, 0, 200], [0.2, 0, 0.2]);
  const bgColor = useTransform(x, [-200, 0, 200], ["#ef4444", "#ffffff", "#10b981"]);
    ...
    <div className="relative w-full h-[78vh] flex flex-col items-center justify-center touch-none">
      <AnimatePresence>
        ...
        <motion.div
          ...
          className="absolute w-full h-full bg-white rounded-[3.5rem] shadow-2xl overflow-hidden cursor-grab active:cursor-grabbing border border-black/5"
        >
          {/* Action Labels */}
          <motion.div style={{ opacity: likeOpacity }} className="absolute top-10 left-10 z-40 border-4 border-emerald-500 text-emerald-500 font-black px-6 py-2 rounded-2xl -rotate-12 uppercase text-3xl">MĂ BAG</motion.div>
          <motion.div style={{ opacity: dislikeOpacity }} className="absolute top-10 right-10 z-40 border-4 border-red-500 text-red-500 font-black px-6 py-2 rounded-2xl rotate-12 uppercase text-3xl">PAS</motion.div>

          <div className="relative h-full w-full">
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent z-10" />
            
            <div className="w-full h-full bg-zinc-200 flex items-center justify-center relative" onClick={nextPhoto}>
              {(() => {
                let images = [];
                ...
                return currentImg ? (
                  ...
                  <div className="flex flex-col items-center gap-2 opacity-10">
                     <Truck size={100} className="text-black" />
                     <p className="text-xs font-black uppercase tracking-[0.5em]">No Media</p>
                  </div>
                );
              })()}
              ...
              <div className="absolute top-4 left-6 right-6 flex gap-1.5 z-30">
                {(() => {
                   ...
                   return images.map((_: any, i: number) => (
                     <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i === currentImageIndex ? 'bg-white' : 'bg-white/30'}`} />
                   ));
                })()}
              </div>
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-10 z-20 space-y-4">
              <div className="flex items-center gap-4">
                <h3 className="text-4xl font-black text-white tracking-tighter leading-none italic">{currentUser.name}, {currentUser.age}</h3>
                {currentUser.isAdmin && (
                  <div className="bg-[#f59e0b] text-black text-[9px] font-black px-3 py-1 rounded-lg shadow-xl">ELITE A</div>
                )}
              </div>
              
              <div className="flex flex-wrap gap-2">
                {currentUser.routeStart && (
                  <div className="flex items-center gap-2 text-[9px] font-black uppercase bg-white/20 backdrop-blur-md px-4 py-2 rounded-xl border border-white/20 tracking-widest text-white">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <span className="text-[9px] font-black text-white/50 uppercase tracking-[0.3em]">Conectat acum</span>
              </div>

              <p className="text-sm font-medium text-white/90 line-clamp-2 leading-relaxed">
                {currentUser.bio || "Împreună pe marile coridoare europene."}
              </p>
            </div>
          </div>
        </motion.div>
