...
export default async function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  if (session) {
    try {
      ...
      console.error('MainLayout user fetch failed:', e);
      // If DB fails, we might still want to show AuthInterface to allow login/setup
    }
  }
      ...
      <header className="flex-none bg-[#020617]/40 backdrop-blur-2xl border-b border-white/[0.03] px-6 py-5 flex items-center justify-between z-40">
        <div className="flex items-center gap-3">
          <Link href="/" className="relative group">
            ...
            <div className="relative bg-gradient-to-br from-red-500 to-red-700 p-2 rounded-xl shadow-lg shadow-red-600/20 group-hover:scale-110 transition-transform">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {user?.isAdmin && (
                <Link href="/admin" className="bg-yellow-500 text-black text-[10px] font-black px-2 py-0.5 rounded-md shadow-[0_0_10px_rgba(234,179,8,0.3)] animate-bounce">A</Link>
              )}
            </div>
