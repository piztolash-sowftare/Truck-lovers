...
export default function MapContent({ parkings, currentUserId }: { parkings: any[], currentUserId: any }) {
              ...
              {selectedParkingUsers.length > 0 ? (
                selectedParkingUsers.map((u) => (
                  <div key={u.id} className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/5 hover:border-red-500/30 transition-all group">
                    ...
                    <Link href={`/profile/${u.id}`} className="p-2 bg-red-600 rounded-lg text-white shadow-lg active:scale-90 transition-all">
