...
import { db } from '@/db';
import { 
  users, swipes, globalMessages, parkings, 
  matches, privateMessages, profileVisits, 
  parkingMessages, blocks, feedPosts, 
  feedComments, feedLikes, notifications 
} from '@/db/schema';
...
export async function updateFullProfile(data: { 
  ...
  const userId = await getCurrentUserId();
  if (!userId) return { error: 'Utilizator neautentificat' };
