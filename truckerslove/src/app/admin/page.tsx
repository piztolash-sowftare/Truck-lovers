import { getAllUsers, deleteUser, adminDeletePhoto, getGlobalMessages, adminDeleteGlobalMessage } from '@/app/actions';
import { db } from '@/db';
import { users } from '@/db/schema';
import { eq } from 'drizzle-orm';
import { getSession } from '@/lib/auth';
import { redirect } from 'next/navigation';
import { Trash2, ShieldCheck, UserX, ExternalLink } from 'lucide-react';
...
export default async function AdminPage() {
  ...
  return (
    <div className="p-6 space-y-12 pb-32">
      <div className="flex items-center justify-between bg-gradient-to-br from-yellow-500/10 via-slate-900 to-transparent p-8 rounded-[3rem] border border-yellow-500/20 shadow-2xl relative overflow-hidden group">
        ...
        <div className="z-10">
          <h1 className="text-4xl font-black text-white italic tracking-tighter flex items-center gap-3">
            CONTROL <span className="text-yellow-500">ELITE</span>
          </h1>
          <p className="text-[10px] text-yellow-500 font-black uppercase tracking-[0.4em] mt-2">Administrator Access Only</p>
        </div>
        <div className="w-16 h-16 rounded-[1.5rem] bg-yellow-500 flex items-center justify-center text-black font-black text-2xl shadow-[0_0_30px_rgba(234,179,8,0.4)] z-10">
          A
        </div>
      </div>
