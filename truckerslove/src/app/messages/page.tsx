      ...
      {matches.length === 0 ? (
        ...
        <div className="space-y-4">
          {matches.map((m: any) => (
            <div 
              key={m.id} 
              className="bg-slate-900/50 border border-white/5 p-5 rounded-[2rem] flex items-center gap-4 hover:bg-slate-900 transition-all hover:border-red-500/30 group relative active:scale-[0.99]"
            >
              <Link 
                href={`/profile/${m.otherUser?.id}`}
                className="w-16 h-16 rounded-2xl bg-slate-800 overflow-hidden border-2 border-white/5 relative flex-none shadow-xl ring-2 ring-red-600/10 z-20 hover:scale-105 transition-transform"
              >
                {m.otherUser?.image ? (
              ...
              <Link href={`/messages/${m.id}`} className="flex-1 min-w-0 flex items-center gap-4 h-full">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-black text-white text-base truncate">{m.otherUser?.name}, {m.otherUser?.age}</h3>
                    <div className="flex items-center gap-1 bg-red-600/10 px-2 py-0.5 rounded-lg border border-red-500/20">
                      <Truck size={8} className="text-red-500" />
                      <span className="text-[7px] text-red-500 font-black uppercase">Elite</span>
                    </div>
                  </div>
                  <p className="text-xs text-slate-400 font-medium truncate italic opacity-60">
                    {m.lastMsg ? m.lastMsg.content : 'Așteaptă un semnal...'}
                  </p>
                </div>
