...
export default async function UserProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#020617] pb-32">
      ...
      <div className="relative h-[65vh] w-full bg-slate-950">
        <Link 
          href="/messages" 
          className="absolute top-6 left-6 z-30 w-12 h-12 rounded-2xl bg-black/40 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:bg-black/60 transition-all"
        >
          <ArrowLeft size={24} />
        </Link>
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="flex items-center gap-4">
                ...
                {user.isAdmin && (
                   <div className="bg-gradient-to-br from-yellow-400 to-yellow-600 text-black text-[12px] font-black px-3 py-1 rounded-xl shadow-[0_0_20px_rgba(234,179,8,0.5)] animate-pulse border-2 border-yellow-300/50">A</div>
                )}
              </div>
              <div className="flex items-center gap-2 mt-4">
                <div className="flex items-center gap-1.5 text-[10px] font-black uppercase text-red-500 bg-red-600/10 px-4 py-2 rounded-full border border-red-500/20 tracking-[0.2em]">
                  <ShieldAlert size={12} />
